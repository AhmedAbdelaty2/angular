...
import {
  markViewForRefresh as util_markViewForRefresh,
  storeLViewOnDestroy,
...
export function isViewDirty(view: ViewRef<unknown>): boolean {
  return requiresRefreshOrTraversal(view._lView);
}

export function markForRefresh(view: ViewRef<unknown>): void {
  util_markViewForRefresh(view['_cdRefInjectingView'] || view._lView);
}
